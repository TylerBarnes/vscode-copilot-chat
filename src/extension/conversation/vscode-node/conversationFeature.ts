...
export class ConversationFeature implements IExtensionContribution {
	...
	set enabled(value: boolean) {
		...
		vscode.commands.executeCommand('setContext', 'acp.copilot.interactiveSession.disabled', !value);
	}
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			vscode.commands.registerCommand('acp.copilot.interactiveSession.feedback', async () => {
				return vscode.env.openExternal(vscode.Uri.parse(FEEDBACK_URL));
			}),
			vscode.commands.registerCommand('acp.copilot.terminal.explainTerminalLastCommand', async () => this.triggerTerminalChat({ query: `/${TerminalExplainIntent.intentName} #terminalLastCommand` })),
			vscode.commands.registerCommand('acp.copilot.terminal.fixTerminalLastCommand', async () => generateTerminalFixes(this.instantiationService)),
			vscode.commands.registerCommand('acp.copilot.terminal.generateCommitMessage', async () => {
			...
			vscode.commands.registerCommand('acp.copilot.git.generateCommitMessage', async (rootUri: vscode.Uri | undefined, _: vscode.SourceControlInputBoxValueProviderContext[], cancellationToken: vscode.CancellationToken | undefined) => {
			...
			vscode.commands.registerCommand('acp.copilot.git.resolveMergeConflicts', async (...resourceStates: (vscode.Uri | vscode.SourceControlResourceState)[]) => {
			...
			vscode.commands.registerCommand('acp.copilot.devcontainer.generateDevContainerConfig', async (args: DevContainerConfigGeneratorArguments, cancellationToken = new vscode.CancellationTokenSource().token) => {
				return this.devContainerConfigurationService.generateConfiguration(args, cancellationToken);
			}),
			vscode.commands.registerCommand('acp.copilot.chat.openUserPreferences', async () => {
		...
		return combinedDisposable(
			vscode.window.registerTerminalQuickFixProvider('copilot-chat.fixWithCopilot', {
				provideTerminalQuickFixes(commandMatchResult, token) {
					...
					return [
						{
							command: 'acp.copilot.terminal.fixTerminalLastCommand',
							title: vscode.l10n.t('Fix using Copilot')
						},
						{
							command: 'acp.copilot.terminal.explainTerminalLastCommand',
			...
			vscode.window.registerTerminalQuickFixProvider('copilot-chat.generateCommitMessage', {
				provideTerminalQuickFixes: (commandMatchResult, token) => {
					return this.enabled ? [{
						command: 'acp.copilot.terminal.generateCommitMessage',
...
function registerSearchIntentCommand(): IDisposable {
	return vscode.commands.registerCommand('acp.copilot.executeSearch', async (arg: FindInFilesArgs) => {
